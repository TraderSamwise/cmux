...
final class QuickTerminalController: NSObject, NSWindowDelegate {
    ...
    /// Load quick terminal settings from the Ghostty config files.
    func loadConfiguration() {
        ...
        var foundKeybind: QuickTerminalKeybind?
        var foundPosition: Position?
        var foundDuration: TimeInterval?
        var foundScreenFraction: CGFloat?
        ...
        for path in configPaths {
            ...
            for line in contents.components(separatedBy: .newlines) {
                ...
                switch key {
                case "keybind":
                    if let parsed = Self.parseKeybind(value) {
                        foundKeybind = parsed
                    }
                case "quick-terminal-position":
                    foundPosition = Position(rawValue: value)
                ...
                case "quick-terminal-screen-fraction":
                    if let frac = Double(value) {
                        foundScreenFraction = CGFloat(frac)
                    }
                default:
                    break
                }
        ...
        if let p = foundPosition { position = p }
        if let d = foundDuration { animationDuration = d }
        if let f = foundScreenFraction { screenFraction = max(0.1, min(1.0, f)) }
    }
